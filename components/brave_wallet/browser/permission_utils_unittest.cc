...
TEST(PermissionUtilsUnitTest, GetConcatOriginFromWalletAddresses) {
  ...
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    EXPECT_TRUE(
        ...
        << "case: " << i;
    EXPECT_EQ(out_origin, GURL(cases[i].expected_out_origin)) << "case: " << i;

    EXPECT_FALSE(GetConcatOriginFromWalletAddresses(GURL(""), cases[i].addrs,
                                                    &out_origin))
        << "case: " << i;
    ...
    EXPECT_TRUE(GetConcatOriginFromWalletAddresses(GURL("https://test.com:123"),
                                                   cases[i].addrs, &out_origin))
        << "case: " << i;
    EXPECT_EQ(out_origin, GURL(cases[i].expected_out_origin_with_port))
        << "case: " << i;
  }
}
  ...
  struct {
    ...
    const char* expected_out_url;
               ...
               {{"BrG44HdsEhzapvs8bEqzvkq4egwevS3fRE6ze2ENo6S8",
                ...
                "JDqrvDz8d8tFCADashbUKQDKfJZFobNy13ugN65t1wvV&origin=https://"
                "a.test.com:123&etld-plus-one=test.com#connectWithSite"}};
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
